...
void notifyWindowClosed() {
  if (g_jvm) {
    ...
    if (g_jvm->GetEnv(reinterpret_cast<void **>(&currentEnv), JNI_VERSION_1_6) == JNI_OK && currentEnv) {
      jclass logOverlayClass = currentEnv->FindClass("io/kitsuri/mayape/service/LogOverlay");
      if (logOverlayClass) {
        jmethodID getInstanceMethod = currentEnv->GetStaticMethodID(logOverlayClass, "getInstance", "(Landroid/content/Context;)Lio/kitsuri/mayape/service/LogOverlay;");
